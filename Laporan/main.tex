...
\begin{figure}[H]
  ...
  \caption{Hasil pra-pemrosesan: (1) Konversi ke Grayscale; (2) Menambahkan \textit{salt and pepper noice}; (3) Menerapkan \textit{Iterative median filtering}}
...
Akan tetapi, jika dilihat berdasarkan nilai \textit{Recall}-nya, SVM dengan Chi-square menghasilkan nilai {Recall} terbaik, yaitu 0.46, dibandingkan dengan Random Forest yang hanya sebesar 0.29 yang mana nilai Recall merupakan nilai yang paling penting untuk kasus medis.

\subsubsection{Dampak Ketidakseimbangan Kelas}
Isu paling kritis pada seluruh eksperimen adalah rendahnya \textit{recall} kelas \textit{malignant}. Hal ini menunjukkan bahwa sebagian besar kasus melanoma gagal terdeteksi. Ketidakseimbangan jumlah sampel membuat model cenderung bias ke kelas mayoritas. Upaya penanganan melalui SMOTE meningkatkan keseimbangan data latih, tetapi belum memberikan perbaikan signifikan pada performa, terutama untuk SVM.
...
Penelitian ini menunjukkan bahwa pendekatan ekstraksi fitur berbasis ABCD (Asimetri, Border, Color, Diameter) yang dikombinasikan dengan fitur tekstur (GLCM, LBP) serta histogram warna HSV mampu menghasilkan representasi yang informatif untuk tugas klasifikasi melanoma pada dataset ISIC 2016. Setelah proses ekstraksi fitur, dua metode seleksi fitur, Chi-square dan PCA, digunakan untuk mereduksi dimensi dan mengevaluasi pengaruhnya terhadap performa model.

Berdasarkan seluruh eksperimen yang dilakukan, model dengan performa keseluruhan terbaik diperoleh pada skenario \textit{Random Forest} menggunakan seluruh 65 fitur dengan penerapan SMOTE, yang mencapai akurasi sebesar 0.7889 dan f1-score kelas \textit{malignant} sebesar 0.34. Meskipun demikian, jika dilihat dari nilai \textit{Recall}, SVM dengan seleksi fitur Chi-square memberikan kinerja terbaik dengan nilai \textit{Recall} sebesar 0.46, lebih tinggi dibandingkan dengan Random Forest yang hanya mencapai 0.29. Hal ini penting karena metrik \textit{Recall} memiliki peran krusial dalam konteks medis, khususnya untuk mendeteksi melanoma secara sensitif agar mengurangi risiko kasus berbahaya yang terlewat.

Metode Chi-square berhasil memilih fitur yang paling relevan, tetapi jumlah fitur yang terlalu sedikit menyebabkan hilangnya sebagian informasi pendukung klasifikasi. PCA mampu mempertahankan 95\% keragaman data, tetapi transformasi linier yang dihasilkan tidak sepenuhnya sejalan dengan struktur diskriminatif yang dibutuhkan oleh model SVM maupun Random Forest.

Secara keseluruhan, beberapa poin utama dapat disimpulkan. Pertama, kombinasi fitur ABCD, tekstur, dan histogram warna memberikan fondasi representasi yang kuat untuk klasifikasi melanoma. Kedua, reduksi dimensi berhasil meningkatkan nilai \textit{Recall}, meskipun memiliki akurasi yang lebih rendah, dibandingkan dengan yang menggunakan seluruh fitur.

Temuan ini membuka peluang untuk pengembangan lanjutan menggunakan pendekatan seperti \textit{deep feature extraction}, \textit{transfer learning}, atau teknik augmentasi data yang lebih bervariasi untuk meningkatkan kemampuan model dalam mendeteksi melanoma secara lebih akurat pada masa mendatang.
